...
export async function buildServer(input?: {
  ...
  server.post("/v1/orgs/:orgId/sessions", async (request, reply) => {
    ...
    if (engineId === "codex.sdk.v1" && resolvedLlm.provider !== "openai" && resolvedLlm.provider !== "openai-codex") {
      throw badRequest("codex.sdk.v1 sessions support only openai/openai-codex providers.");
    }
