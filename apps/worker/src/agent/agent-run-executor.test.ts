...
describe("agent.run executor", () => {
  ...
  it("includes resolved toolset payload for claude.agent-sdk.v1 remote agent.run and emits agent_toolset_applied", async () => {
    const loadSecretValue = vi.fn(async () => "sk-secret");
    const loadToolsetById = vi.fn(async () => ({
      id: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
      name: "My Toolset",
      mcpServers: [{ name: "ext", transport: "stdio", command: "echo", args: ["hi"], env: { TOKEN: "${ENV:TEST_TOKEN}" } }],
      agentSkills: [{ format: "agentskills-v1", id: "hello", name: "Hello", entry: "SKILL.md", files: [{ path: "SKILL.md", content: "# hi" }] }],
    }));
